...
def _iter_parsed_labels(labels) -> typing.Generator[ocm.Label, None, None]:
    '''
...
def append(parsed):
    ...
    def inject_labels(artefact: dict):
        if not (labels := parsed.labels):
            return

        labels = list(_iter_parsed_labels(labels=labels))
        if not 'labels' in artefact:
            artefact['labels'] = []

        artefact['labels'] = [dataclasses.asdict(l) for l in labels]

    if isinstance(obj, list):
        for o in obj:
            inject_labels(artefact=o)

        attr.extend(obj)
    else:
        inject_labels(artefact=obj)
        attr.append(obj)
